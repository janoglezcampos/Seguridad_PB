...
public class Util {
				...
				try {
					...
					if (res.getError() != 0) {
						...
						if (Validation.validateCert(certFirma, Client.getTrust())) {
							byte[] SignRDContent = Validation.getSignRDContent(res.getIdRegistro(),
									res.getSelloTemporal(), res.getIdPropietario(), fileContent, message.get(1));

							if (Validation.checkSign(certFirma, SignRDContent, res.getSigRD())) {
								Client.saveHash(res.getIdRegistro(), fileContent);
								System.out.println("Documento correctamente registrado con el numero con ID: "
										+ res.getIdRegistro());
	...
	public static ArrayList<byte[]> generateMessage(String passwd_key, String ubicacion, String confidencialidad)
			throws Exception {
