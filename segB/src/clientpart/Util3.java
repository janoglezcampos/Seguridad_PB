		...
		try {
			...
			if (response.getError() == 0) {
				if (Validation.validateCert(response.getCert(), Client.getTrust())) {
					...
					if (response.getIsPrivate()) {
						try {
							file.write(fileContent = Encription.decriptFilePGP(response.getEncriptedKey(),
									response.getEncriptedFile(), response.getCipherParams(), pass_wd, CIPHER_ALIAS,
									Client.getKeyStore()));
					...
					PrivateKey signkey = (PrivateKey) Client.getKeyStore().getKey(SIGN_ALIAS, pass_wd.toCharArray());

					// Si se comprueba
					byte[] firmadoc = Validation.signContent(fileContent, signkey);
					SignRDContent = Validation.getSignRDContent(response.getIdRegistro(), response.getSelloTemporal(),
							response.getIdPropietario(), fileContent, firmadoc);
					/*
